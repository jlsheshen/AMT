...
public class SubjectImageLoader {
	...
	public void preloadAllPics() {
		...
		try {
			...
			sql = "SELECT QUESTION FROM TB_SUBJECT";
			curs = db.rawQuery(sql, null);
			parseRichCursor(curs, urls);
	 ...
	 * @param urls
	 */
	private void parseRichCursor(Cursor curs, List<String> urls) {
		if (curs != null) {
			while (curs.moveToNext()) {
				RichTextData richData = RichTextUtil.parse(curs.getString(0));
				List<String> imgs = richData.getImgSrc();
				if (imgs != null) {
					for (int i = 0; i < imgs.size(); i++) {
						String url = imgs.get(i);
						if (!urls.contains(url) && URLUtil.isNetworkUrl(url)) {
							urls.add(url);
						}
					}
				}
	...
	public void preDownloadPic(final String uri) {
		...
		mImageLoader.loadImage(uri, new SimpleImageLoadingListener() {
			...
			public void onLoadingFailed(String imageUri, View view, FailReason failReason) {
				String reason = parseErrorMsg(failReason.getType(), uri);
	 ...
	 * @return
	 */
	private String parseErrorMsg(FailType type, String uri) {
		String reason = null;
		switch (type) {
		case IO_ERROR:
			reason = "图片输入输出错误";
