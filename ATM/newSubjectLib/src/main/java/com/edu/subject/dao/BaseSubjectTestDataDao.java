 ...
 * 
 */
public abstract class BaseSubjectTestDataDao extends BaseDataDao2 {
	...
	protected BaseSubjectTestDataDao(Context context) {
	...
	private BaseTestData initComprehensiveSubject(Cursor curs, CommonSubjectData subjectData, int testMode) {
		...
		//加载子题
		List<CommonSubjectData> subjects = CommonSubjectDataDao.getInstance(mContext).getDatasByParentId(subject.getId());
	...
	public void parseCursor(Cursor curs, BaseTestData data, int index) {
		if (curs != null) {
			...
			if (data.getTestMode() == TestMode.MODE_EXAM_EMPTY) {// 测试模式不加载用户数据
				...
				data.setuScore(curs.getInt(4));
				data.setState(curs.getInt(5));

				//用户答案初始化
				String uAnswer = curs.getString(3);
