...
import com.edu.library.util.LogUtil;
 ...
 */
public class EduCrashHandler implements UncaughtExceptionHandler {
	public static final String TAG = "CrashHandler";

	// 系统默认的UncaughtException处理类
	private UncaughtExceptionHandler mDefaultHandler;
	// CrashHandler实例
	private static EduCrashHandler INSTANCE = new EduCrashHandler();
	// 程序的Context对象
	...
	private EduCrashHandler() {
	}

	/** 获取CrashHandler实例 ,单例模式 */
	public static EduCrashHandler getInstance() {
		return INSTANCE;
	}
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				Toast.makeText(mContext, "很抱歉,程序出现异常,即将退出,请与管理员联系！", Toast.LENGTH_LONG).show();
				Looper.loop();
			}
		}.start();
		// 收集设备参数信息
