...
import com.edu.accountingteachingmaterial.fragment.ReviewViewPagerFragment;
 ...
 * 
 */
public class ReviewSubjectViewPagerAdapter extends FragmentPagerAdapter {
	 ...
	 * 对应界面集合
	 */
	private ArrayList<ReviewViewPagerFragment> mPagerList = new ArrayList<ReviewViewPagerFragment>();
	...
	public ReviewSubjectViewPagerAdapter(FragmentManager childFragmentManager, List<BaseTestData> datas, Context context, SubjectListener listener) {
	...
	public Fragment getItem(int position) {
		...
		if (mPagerList.get(position) == null) {
			mPagerList.set(position, ReviewViewPagerFragment.newInstance(mSubjectList.get(position), mListener));
	...
	public void destroyItem(ViewGroup container, int position, Object object) {
		try {
			// 销毁fragment
			ReviewViewPagerFragment fragment = mPagerList.get(position);
			fragment = null;
	...
	public float submit(int index) {
		...
		ReviewTestDataDao.getInstance(mContext).updateTestData(mSubjectList.get(index));
...
//			mSubjectList.get(i).setuScore(0);
		...
		Log.d(TAG, "totalScore:" + totalScore);
		ReviewTestDataDao.getInstance(mContext).updateTestDatas(mSubjectList);
	...
	private void submitSubject(int index) {
		ReviewViewPagerFragment pager = mPagerList.get(index);
		if (pager != null) {
			pager.submit();
		}
