...
import static com.edu.subject.SubjectState.STATE_WRONG;
 ...
 * 
 */
public class SubjectOnlineTestDataDao extends BaseDataDao2 {
String TAG = "SubjectOnlineTestDataDao";
	//题目类型
	 ...
	 * @return
	 */
	public static SubjectOnlineTestDataDao getInstance(Context context) {
		if (instance == null)
...
//				data.setUAnswer(curs.getString(curs.getColumnIndex(UANSWER)));
				...
				if (data.getTestMode() == TestMode.MODE_SHOW_DETAILS && data.getState() == STATE_WRONG){
					ErrorTestDataDao.getInstance(mContext).insertTest(data.getSubjectId());
				}
				//用户答案初始化
