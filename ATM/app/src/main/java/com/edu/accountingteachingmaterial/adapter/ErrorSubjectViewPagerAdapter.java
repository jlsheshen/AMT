...
import com.edu.accountingteachingmaterial.fragment.ErrorViewPagerFragment;
 ...
 * 
 */
public class ErrorSubjectViewPagerAdapter extends FragmentPagerAdapter {
	 ...
	 * 对应界面集合
	 */
	private ArrayList<ErrorViewPagerFragment> mPagerList = new ArrayList<ErrorViewPagerFragment>();
	...
	public ErrorSubjectViewPagerAdapter(FragmentManager childFragmentManager, List<BaseTestData> datas, Context context, SubjectListener listener) {
	...
	public Fragment getItem(int position) {
		...
		if (mPagerList.get(position) == null) {
			mPagerList.set(position, ErrorViewPagerFragment.newInstance(mSubjectList.get(position), mListener));
	...
	public void destroyItem(ViewGroup container, int position, Object object) {
		try {
			// 销毁fragment
			ErrorViewPagerFragment fragment = mPagerList.get(position);
			fragment = null;
	...
	public float submit(int index) {
		...
		ErrorTestDataDao.getInstance(mContext).updateTestData(mSubjectList.get(index));
...
//			mSubjectList.get(i).setuScore(0);
		...
		Log.d(TAG, "totalScore:" + totalScore);
		ErrorTestDataDao.getInstance(mContext).updateTestDatas(mSubjectList);
	...
	private void submitSubject(int index) {
		ErrorViewPagerFragment pager = mPagerList.get(index);
		if (pager != null) {
			pager.submit();
		}
