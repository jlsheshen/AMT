 ...
 * 普通的查看答案页面
 */
public class OnlineExamDetailsContentActivity extends FragmentActivity implements OnItemClickListener, OnCardItemClickListener {
...
//        ExamListData data = (ExamListData) bundle.get("ExamListData");
        ...
        for (BaseTestData data : datas) {
            if (data.getState() == SubjectState.STATE_WRONG) {
                ErrorTestDataDao.getInstance(this).insertTest(data.getSubjectType(), Integer.parseInt(data.getSubjectId()), data.getSubjectData().getChapterId());
            }
        }
