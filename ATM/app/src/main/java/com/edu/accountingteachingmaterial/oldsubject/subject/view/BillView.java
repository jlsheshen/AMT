package com.edu.accountingteachingmaterial.oldsubject.subject.view;
...
public class BillView extends RelativeLayout implements ISubject, BillZoomListener, SignViewListener, OnClickListener, CloseListener {
    ...
    public BillView(Context context, TestBillData data, boolean isGroup) {
        super(context);
    ...
    private void init() {
        ...
        if (!isGroup) {
            ...
            subTypeTv.setText("表格 - " + mData.getSubjectIndex());
            scoreTv.setText("(" + (int) mData.getSubjectData().getScore() + "分)");
        }
