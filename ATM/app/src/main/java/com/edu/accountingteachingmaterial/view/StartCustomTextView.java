...
//  private float lineSpacingMult = 0.5f;
    ...
    private int measureContentHeight(int width)
    {
        ...
        for (int i = 0; i < obList.size(); i++)
        {
            ...
            //此处做出修改,增加了最大行数限制
            if (width - drawedWidth < obWidth && contentList.size()<this.getMaxLines())
            {
        ...
        if (line != null && line.line.size() > 0&& contentList.size()<this.getMaxLines())
        {
            contentList.add(line);
    ...
    public void setMText(CharSequence cs)
    {
        ...
        ArrayList<IS> isList = new ArrayList<IS>();
        useDefault = false;
